...
/* 사용자 정의 예외
 	...
 	2. RuntimeException 을 상속하는 예외 : UnChecked Exception - 실행시에 예외
*/
...
class A {
	...
	void abc_1(int num) {
		...
		} catch(MyException e) {	// me1 의 강제로 throw 를 사용해서 강제로 예외 발생시킴 
			System.out.println("70 이하 값을 넣었습니다. 70 이상 값을 넣으세요");
			System.out.println(e.getMessage());
		}
	}
	
	// 예외의 전가 ( throws ) : abc_2() 메소드 호출하는 곳에서 예외 처리
	void abc_2(int num) throws MyException {
		if (num > 70) {
			System.out.println("정상 작동");
		} else {
			throw me1;		// throw 는 강제 예외 발생 
		}
	}
...
public class CreateUserException_1 {
	public static void main(String[] args) {
			A a = new A();
			a.bcd_1();			// 예외를 직접 구현
			a.bcd_2(); 			// throws 예외 전가, 호출하는 족에서 얘외 처리
